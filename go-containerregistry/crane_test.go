...
func TestCrane(t *testing.T) {
	...
	path = path + "gocker.tar"
	img, err := crane.Pull(strings.Join([]string{src, "latest"}, ":"))
	if err != nil {
		fmt.Println("rane.Pull failed,err:", err)
		return
	}
	//2、调用crane.SaveLegacy() 此步骤开始下载镜像数据
	err = crane.SaveLegacy(img, src, path)
	if err != nil {
		fmt.Println("rane.SaveLegacy failed,err:", err)
		return
	}
}
